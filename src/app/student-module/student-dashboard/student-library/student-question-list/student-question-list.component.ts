...
export class StudentQuestionListComponent implements OnInit {
  ...
  sortTags(){
    if(this.sortValue){
      ...
      this.sortValue = ""
      this.dropdownField = "relevant"
      this.queryForm["sort"] = "relevant"
      this.checkObj['sort'] = "relevant"
      this.router.navigate([], { queryParams: this.checkObj });
      this.getLibraryListing(this.queryForm, this.payload);
    }
  }
  ...
  getLibraryListing(query: any, payload: any) {
    ...
    this.authservice.getLibrary(this.queryForm, payload).subscribe(
      (data) => {
    ...
    if (!this.fieldOfStudySearchValue) {
      ...
      this.authservice
        ...
        .subscribe((data) => {
          ...
          // (this.sectorSearchData,'search data')
          // this.temp()
        });
    }
  }
  ...
  filterDropdown(data) {
    ...
    // if(this.sortValue){
    //   this.checkObj['sort'] = this.advisorLocationParams
    //   this.router.navigate([], { queryParams: this.checkObj });
    // }
  }
  ...
  messageDetail(id) {
    this.router.navigateByUrl(
      `/student-module/dashboard/student-library/student-question-reply/` + id
    );
  }
  ...
  changeValue(value, ide) {
    ...
    // this.router.navigate([], { queryParams: {sector : this.paramsUrl , employer : this.paramsEmploye , fieldOfStudy : this.paramsFieldOfStudy , university : this.paramsUniversity , typeOfJob : this.paramsTypeOfJob , advisorLocation :  this.advisorLocationParams } });

    this.router.navigate([], { queryParams: this.checkObj });
    this.getLibraryListing(this.queryForm, this.payload);
    // ('API hitting is ending hare')
  }
  ...
  resetSearch() {
    ...
    if (this.paramsYear) {
      ...
      (this.advisorLocationParams ,'this.advisorLocationParams')
    if(this.advisorLocationParams){
      delete this.checkObj['advisorLocation']
    }

    (this.studentLocationParams ,'this.studentLocationParams')
    if(this.studentLocationParams){
