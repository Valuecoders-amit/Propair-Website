...
export class InboxComponent implements OnInit {
  ...
  inboxList() {
    ...
    this.studentService.inbox().subscribe(data => {
      ...
      this.studentService.inboxCount(this.inboxCount)

      // this.inboxData.forEach(item => {
      //   let duration = moment.duration(moment(new Date()).diff(moment(item.updatedAt)));
      //   if (duration['_data'].seconds > 0) {
      //     item['duration'] = `${duration['_data'].seconds} s `;
      //   }
      //   if (duration['_data'].minutes > 0) {
      //     item['duration'] = `${duration['_data'].minutes} m` ;
      //   }
       
      //   if (duration['_data'].days > 0) {
      //     item['duration'] = `${duration['_data'].days} ${(duration['_data'].days) > 1 ? 'd' : 'd'}
      //     ${(duration['_data'].minutes > 0) ? (duration['_data'].minutes == 1) ? `${duration['_data'].minutes} m` : `${duration['_data'].minutes} m` : ''}
