...
@Component({
  selector: 'app-propair-support',
  templateUrl: './propair-support.component.html',
  styleUrls: ['./propair-support.component.scss']
})
export class PropairSupportComponent implements OnInit {
  ...
  thumbnailImage(data){
    ...
    this.studentService.thumbnailImage(payload).subscribe(data=>{
      if(data['status'] == 200){
        let image = data['url']
  ...
  cancel() {
    this.imageUrl = null ;
    this.propairForm.reset();
    this.router.navigateByUrl('/student-module/dashboard/student-my-message/propair-support')
  }
