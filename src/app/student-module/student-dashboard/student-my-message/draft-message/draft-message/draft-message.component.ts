...
export class DraftMessageComponent implements OnInit {
  ...
  public allDraftLiveData :any;
  public allDraftNewData :any;
  ...
  ngOnInit() {
    ...
    this.studentService.message_deletDraft.subscribe(data =>{
      this.deleteDraft = data 
    })
    // this.outbox();
    // this.archive();
    // this.inboxList();
  ...
  draftNewMessageExchange() {
    this.studentService.draftNewMessageExchange().subscribe(data => {
      this.studentService.draftDataNewLists(data)
  ...
  draftbox() {
    this.studentService.draftBox().subscribe(data => {
      this.studentService.draftDataLiveLists(data)
  ...
  deletePopup() {
    ...
    // if (this.draftTab == 'LiveMessageExchange' && this.deleteDraft.length) {
    //   this.selectedMessages = [];
    //   this.draftData.forEach(data => {
    //     if (data.value) {
    //       this.selectedMessages.push(data._id);
    ...
    // }
    // else if (this.draftTab == 'newMessageExchange' && this.deleteDraft.length) {
    //   this.selectedMessages = [];
  ...
  delete() {
    if (this.draftTab == 'LiveMessageExchange'  && this.deleteDraft.length) {
      if (this.deleteDraft) {
        this.studentService.deleteMessageLive(this.deleteDraft).subscribe(data => {
          if (data['status'] == 200) {
      ...
      if (this.deleteDraft) {
        this.studentService.deleteMessage(this.deleteDraft).subscribe(data => {
          if (data['status'] == 200) {
            this.toasterService.success('Success', data['message']);
            $("#delete").modal("hide");
            this.draftNewMessageExchange();
