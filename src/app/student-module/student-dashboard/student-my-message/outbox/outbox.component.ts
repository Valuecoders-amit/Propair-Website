...
export class OutboxComponent implements OnInit {
  ...
  outbox() {
    this.studentService.outBox().subscribe(data => {
      ...
      this.studentService.outBoxCount(this.outboxCount)
      ...
      //   let difftimestamp = new Date(item.sendsIn).getTime() - Date.now();
      //   if(difftimestamp < 0) {
      //     item['sendingIn'] = {
      //       minutes: 0,
      //       seconds: 0
      ...
      //   } else {
      //     let minutes =Math.floor(difftimestamp/60000); 
      //     this.sendIn = moment(new Date(item.sendsIn))
      //     item['sendingIn'] = {
      //       minutes: minutes,
      //       seconds: 60
      //     };
      ...
      //   }
        ...
        // let duration = moment.duration(moment(new Date()).diff(moment(item.sendsIn)));
        // if (duration['_data'].seconds > 0) {
        //   item['duration'] = `${duration['_data'].seconds} s `;
        //   item['sendingIn'].seconds = duration['_data'].seconds;
        // }
        // if (duration['_data'].minutes > 0) {
        //   item['duration'] = `${duration['_data'].minutes}  ${(duration['_data'].minutes) > 1 ? 'm' : 'm'}
        //   ${(duration['_data'].seconds > 0) ? (duration['_data'].seconds == 1) ? `${duration['_data'].seconds} s` : `${duration['_data'].seconds} s` : ''}`;
        //   item['sendingIn'].minutes = duration['_data'].minutes;
        //   item['sendingIn'].seconds = duration['_data'].seconds;
        // }
        // if (duration['_data'].hours > 0) {
        //   item['duration'] = `completed ${duration['_data'].hours} ${(duration['_data'].hours) > 1 ? 'hours' : 'hour'} 
        //   ${(duration['_data'].minutes > 0) ? (duration['_data'].minutes == 1) ? `${duration['_data'].minutes} minute` : `${duration['_data'].minutes} minutes` : ''} ago`;
        // }
        // if (duration['_data'].days > 0) {
        //   item['duration'] = `completed ${duration['_data'].days} ${(duration['_data'].days) > 1 ? 'days' : 'days'} ago`;
        // }
    //   })
    //   this.runTimer();
    // })
  ...
  //       if ((data.sendingIn.seconds - 1) >= 0) {
  //         data.sendingIn.seconds = data.sendingIn.seconds - 1;
  //       } else if ((data.sendingIn.minutes - 1) >= 0) {
  //         data.sendingIn.seconds = 59
  //         data.sendingIn.minutes = data.sendingIn.minutes - 1;
  //       }
  ...
  navigatToNewMessageExchange(id) {
    ...
    this.router.navigateByUrl('/student-module/dashboard/message-exchange/message-exchange1/' + id)
  }
  ...
  newMessageDraftChecked() {
    this.selectedAll = this.draftMessageExchange.every(function (item: any) {
      return item.value == true;
    })
